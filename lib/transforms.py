...
class ChromaticTranslation(object):
    ...
    def __call__(self, coords, feats, labels):
        if random.random() < 0.95:
            tr = (np.random.rand(1, 3) -
                  0.5) * 255 * 2 * self.trans_range_ratio
...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, coords, feats, labels):
        if random.random() < 0.2:
            ...
            ) if self.randomize_blend_factor else self.blend_factor
            feats[:, :3] = (
                1 - blend_factor) * feats + blend_factor * contrast_feats
        return coords, feats, labels
...
class HueSaturationTranslation(object):
    ...
    def __call__(self, coords, feats, labels):
        ...
        feats[:, :3] = np.clip(HueSaturationTranslation.hsv_to_rgb(hsv), 0,
                               255)
...
class ElasticDistortion:
    ...
    def elastic_distortion(self, coords, feats, labels, granularity,
                           magnitude):
    ...
    """
        ...
        noise_dim = (
            (coords - coords_min).max(0) // granularity).astype(int) + 3
        noise = np.random.randn(*noise_dim, 3).astype(np.float32)

        # Smoothing.
        for _ in range(2):
            noise = scipy.ndimage.filters.convolve(noise,
                                                   blurx,
                                                   mode='constant',
                                                   cval=0)
            noise = scipy.ndimage.filters.convolve(noise,
        ...
        ax = [
            np.linspace(d_min, d_max, d) for d_min, d_max, d in zip(
                coords_min - granularity, coords_min + granularity *
                (noise_dim - 2), noise_dim)
        ]
        interp = scipy.interpolate.RegularGridInterpolator(ax,
                                                           noise,
                                                           bounds_error=0,
                                                           fill_value=0)
    ...
    def __call__(self, list_data):
        ...
        for transformation in transformations:
            ...
            transformations_batch.append(
                torch.from_numpy(transformation).float())
            batch_id += 1
