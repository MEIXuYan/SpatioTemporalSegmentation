...
def checkpoint(model,
    ...
    json.dump(vars(config),
              open(config.log_dir + '/config.json', 'w'),
              indent=4)
...
def save_predictions(coords, upsampled_pred, transformation, dataset, config,
    ...
    iter_size = coords[:, -1].max(
    ) + 1  # Normally batch_size, may be smaller at the end.
    ...
    for i in range(iter_size):
        ...
        if dataset.IGNORE_LABELS:
            if isinstance(dataset, OnlineVoxelizationDatasetBase):
                ...
                masked2label = [
                    label2masked.tolist().index(i) for i in range(maskedmax)
                ]
                ...
                for k, v in dataset.label_map.items():
                    decode_label_map[v] = k
                orig_pred = np.array([decode_label_map[x] for x in orig_pred],
                                     dtype=np.int)
...
def visualize_results(coords, input, target, upsampled_pred, config,
                      iteration):
    ...
    ptc_nonpred = np.hstack(
        (input_xyz[target_nonpred], np.zeros((np.sum(target_nonpred), 3))))
    ...
    xyzlabel = colorize_pointcloud(input_xyz[target_pred],
                                   upsampled_pred[target_pred])
    xyzlabel = np.vstack((xyzlabel, ptc_nonpred))
    filename = '_'.join(
        [config.dataset, config.model, 'pred',
         '%04d.ply' % file_iter])
    save_point_cloud(xyzlabel,
                     os.path.join(config.visualize_path, filename),
                     verbose=False)
    # RGB input values visualization.
    xyzrgb = np.hstack(
        (input_xyz[target_batch], input[:, :3].cpu().numpy()[target_batch]))
    filename = '_'.join(
        [config.dataset, config.model, 'rgb',
         '%04d.ply' % file_iter])
    save_point_cloud(xyzrgb,
                     os.path.join(config.visualize_path, filename),
                     verbose=False)
    # Ground-truth visualization in RGB.
    xyzgt = colorize_pointcloud(input_xyz[target_pred],
                                target.numpy()[target_pred])
    xyzgt = np.vstack((xyzgt, ptc_nonpred))
    filename = '_'.join(
        [config.dataset, config.model, 'gt',
         '%04d.ply' % file_iter])
    save_point_cloud(xyzgt,
                     os.path.join(config.visualize_path, filename),
                     verbose=False)
...
def permute_pointcloud(input_coords, pointcloud, transformation, label_map,
    ...
    for i in range(input_batch_size + 1):
        ...
        original_coords1 = np.hstack(
            (pointcloud_b[:, :3], np.ones((pointcloud_b.shape[0], 1))))
        original_vcoords = np.floor(
            original_coords1 @ transformation_b.T)[:, :3].astype(int)
        ...
        input_coords_key = _hash_coords(input_coords_b, vcoords_min,
                                        vcoords_dims)
        original_vcoords_key = _hash_coords(original_vcoords, vcoords_min,
                                            vcoords_dims)
    ...
    pointcloud_output = torch.zeros(pointcloud.shape[0],
                                    voxel_output.shape[1]).to(voxel_output)
    ...
    pointcloud_target = torch.from_numpy(
        np.array([label_map[i]
                  for i in np.concatenate(pointcloud_target)])).int()
