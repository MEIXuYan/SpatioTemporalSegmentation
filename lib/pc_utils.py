...
class Camera(object):
    ...
    """
        if labels is not None:
            ...
            if (labels.ndim == 2) or ((labels.ndim == 3) and
                                      (labels.shape[2] == 1)):
                n_label_channels = 1
        ...
        ones = np.ones_like(A)
        grid = np.concatenate(
            (A[:, :, np.newaxis], B[:, :, np.newaxis], ones[:, :, np.newaxis]),
            axis=2)
        ...
        XYZ = np.concatenate((prod[:2, :].T, Z.flatten()[:, np.newaxis]),
                             axis=1)  # Nx3
        ...
        if prune is True:
            ...
            for idx in range(points_3d.shape[0]):
                ...
                if (cur_z == 0) or (
                        cur_z == 65535
                ):  # Don't show things at 0 distance or max distance
                    continue
    ...
    """
        ...
        if orig_points_3d.shape[1] == 6:  # XYZRGB
            points_3d_world = np.hstack(
                (points_3d_world, orig_points_3d[:, -3:]))
        return points_3d_world
    ...
    """
        return self._transform_points(points_3d, extrinsics,
                                      self._camera2world_transform)

    def world2camera(self, extrinsics, points_3d):
        """Transform from world coordinates (3D) to camera coordinates (3D).
    """
        return self._transform_points(points_3d, extrinsics,
                                      self._world2camera_transform)
...
def colorize_pointcloud(xyz, label, ignore_label=255):
    assert label[label != ignore_label].max() < len(
        COLOR_MAP_RGB), 'Not enough colors.'
    label_rgb = np.array([
        COLOR_MAP_RGB[i] if i != ignore_label else IGNORE_COLOR for i in label
    ])
...
class PlyWriter(object):
    ...
    POINTCLOUD_DTYPE = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('red', 'u1'),
                        ('green', 'u1'), ('blue', 'u1')]
    ...
    def write_vertex_only_ply(cls, vertices, filepath):
        ...
        pc_points = np.array(
            [tuple(p) for p in np.concatenate((vertices, color), axis=1)],
    ...
    def write_ply_vert_color(cls, vertices, colors, filepath):
        # assume that points are N x 3 np array for vertex locations
        pc_points = np.array(
            [tuple(p) for p in np.concatenate((vertices, colors), axis=1)],
            dtype=cls.POINTCLOUD_DTYPE)
        cls.write_ply(pc_points, filepath)
    ...
    def concat_label(cls, target, xyz, label):
        ...
        subpointcloud = np.array([tuple(l) for l in subpointcloud],
                                 dtype=cls.POINTCLOUD_DTYPE)
