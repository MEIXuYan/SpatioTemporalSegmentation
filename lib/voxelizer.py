...
class Voxelizer:
    ...
    def get_transformation_matrix(self):
        ...
        if self.use_augmentation and self.rotation_augmentation_bound is not None:
            if isinstance(self.rotation_augmentation_bound,
                          collections.Iterable):
                rot_mats = []
                for axis_ind, rot_bound in enumerate(
    ...
    def voxelize(self, coords, feats, labels, center=None):
        assert coords.shape[1] == 3 and coords.shape[0] == feats.shape[
            0] and coords.shape[0]
    ...
    def voxelize_temporal(self,
        ...
        # ######################### Voxelization #############################
        # Voxelize coords
        for coords, feats, labels, center in zip(coords_t, feats_t, labels_t,
                                                 centers):
